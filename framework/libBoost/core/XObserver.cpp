...
XObserver* XObserver::_instance = NULL;
...
bool XObserver::listen(std::string name, boost::function<void(void*)> callBack)
{
...
{
	std::map<std::string,boost::function<void(void*)>>::iterator it = this->keyMap.find(name);
	if (it->first!="")
	{
...
bool XObserver::notify(std::string name, void* param)
{
	...
	{
		boost::function<void(void*)> callBack = it->second;
		callBack(param);
		return true;
	}
	return false;
